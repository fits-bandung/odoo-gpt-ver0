...
st.set_page_config(page_title='Odoo-GPT', layout='wide', initial_sidebar_state='auto')
...
for i, sublist in enumerate(st.session_state.stored_session):
        with st.sidebar.expander(label= f"Conversation-Session:{i}"):
            st.write(sublist)
            st.download_button("Download Chat", "\n".join(sublist), file_name=f"chat-{i}.txt", mime="text/plain")

# Allow the user to clear all stored conversation sessions
if st.session_state.stored_session:   
    # if st.sidebar.checkbox("Clear-all"):
    if st.sidebar.button("Clear-all"):
        st.sidebar.checkbox("Klik disini untuk menghapus semuanya", on_change=lambda: st.session_state.clear())
